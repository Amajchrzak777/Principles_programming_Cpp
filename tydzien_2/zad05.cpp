...
namespace MojNamespace { //moja przestrzen nazw do wczytywania exit zeby
	const char exit = 'Q'; // zapobiec powielieniu nazw
}
...
Token Token_stream::get()
{
	...
	default:
		if (isalpha(ch)/* || ch == '_'*/) {
			...
			if (s == "sqrt") return Token(square_ex); //dla pierwiastka
			if (s == "pow") return Token(pow_ex); //dla potegowania
			return Token(name, s);
		}
		error("Bad token");
	}
...
double square()
{
	double d = expression();		//funkcja dotycząca pierwiastka
